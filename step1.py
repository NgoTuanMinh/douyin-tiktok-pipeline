...
def process_one_video(task):
    ...
    download_video_direct(url, temp_video)
    ...
    script_path = None
    if has_audio:
        print(f"  - Transcribe tiếng Trung...")
        transcript = transcribe_audio(str(final_video))
        
        print(f"  - Dịch sang tiếng Việt (qua Gemini web)...")
...
def main():
    ...
    ensure_dirs([STEP1_OUTPUT, VIDEO_DIR, SCRIPT_DIR, INPUT_JSON_DIR])
    ...
    try:
        ...
        # Đảm bảo đóng Gemini client sau khi xử lý xong
        close_gemini_client()
